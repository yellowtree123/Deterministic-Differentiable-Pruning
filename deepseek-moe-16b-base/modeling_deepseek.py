...
class DeepseekMLP(nn.Module):
    ...
    def forward(self, x, expert_z=None):
        tp = self.config.pretraining_tp

        if tp > 1:
            slice_size = self.intermediate_size // tp
            ...
            gate_proj = torch.cat(
                [F.linear(x, gate_proj_slices[i]) for i in range(tp)],
                dim=-1,
            )
            up_proj = torch.cat(
                [F.linear(x, up_proj_slices[i]) for i in range(tp)],
                dim=-1,
            )

            hidden = self.act_fn(gate_proj) * up_proj

            # === match no-TP behavior ===
            if expert_z is not None:
                # convert expert_z to hidden data type
                expert_z = expert_z.to(hidden.dtype)
                hidden = hidden * expert_z  # broadcast OK (e.g., [1,1,intermediate] or [bs,seq,intermediate])

            hidden_slices = hidden.split(slice_size, dim=-1)

            down_proj = 0
            for i in range(tp):
                down_proj = down_proj + F.linear(hidden_slices[i], down_proj_slices[i])
        else:
            if expert_z is not None:
                current_hidden_states = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
                expert_z = expert_z.to(current_hidden_states.dtype)
                current_hidden_states = current_hidden_states *  expert_z
...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states, expert_z=None):
        ...
        if self.training:
            ...
            if first_param is not None and hidden_states.dtype != first_param.dtype:
                hidden_states = hidden_states.to(first_param.dtype)
                target_dtype = first_param.dtype

            y = torch.empty(hidden_states.shape, device=hidden_states.device, dtype=torch.bfloat16)
            hidden_states = hidden_states.to(target_dtype)
            for i, expert in enumerate(self.experts):
                y[flat_topk_idx == i] = expert(hidden_states[flat_topk_idx == i], expert_z=expert_z[i] if expert_z is not None else None)
            y = (y.view(*topk_weight.shape, -1) * topk_weight.unsqueeze(-1)).sum(dim=1)
            y =  y.view(*orig_shape)
            y = AddAuxiliaryLoss.apply(y, aux_loss)
        else:
            y = self.moe_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1), expert_z=expert_z if expert_z is not None else None).view(*orig_shape)
        if self.config.n_shared_experts is not None:
            y = y + self.shared_experts(identity)
        return y
    
    @torch.no_grad()
    def moe_infer(self, x, flat_expert_indices, flat_expert_weights, expert_z=None):
        ...
        for i, end_idx in enumerate(tokens_per_expert):
            ...
            expert_out = expert(expert_tokens, expert_z=expert_z[i] if expert_z is not None else None)
...
class DeepseekDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        expert_z: Optional[torch.Tensor] = None,
        **kwargs,
        ...
        if isinstance(self.mlp, DeepseekMLP):
            hidden_states = self.mlp(hidden_states)
        else:
            hidden_states = self.mlp(hidden_states, expert_z=expert_z)
...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        expert_z: Optional[torch.FloatTensor] = None,
        ...
        for decoder_layer in self.layers:
            ...
            if expert_z is None:
                ...
                if layer_num==0:
                    ...
                    layer_expert_z = expert_z[layer_num-1,:,:]
